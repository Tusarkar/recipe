(function () {
  ...
  var IMG = {
    hero:
      "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=900&q=85&fm=jpg",
    pizza:
      "https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?w=900&q=85&fm=jpg",
    burger:
      "https://images.unsplash.com/photo-1555939594-58d7cb561ad1?w=900&q=85&fm=jpg",
    pasta:
      "https://images.unsplash.com/photo-1473093295043-cdd812d0e601?w=900&q=85&fm=jpg",
    pancakes:
      "https://images.unsplash.com/photo-1567620905732-2d1ec7ab7445?w=900&q=85&fm=jpg",
    bowl:
      "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=900&q=85&fm=jpg",
  };
  ...
  var orderUnavailableToast = document.getElementById("orderUnavailableToast");
  var orderToastHideTimer = null;
  var orderToastRemoveTimer = null;
  var filter = "all";
  ...
  function openModal(recipe) {
    ...
    modalImgEl.src = recipe.image;
    modalImgEl.alt = recipe.title;
  ...
  function syncScrollLock() {
    var locked = !modal.hidden;
    document.body.style.overflow = locked ? "hidden" : "";
  }

  function showOrderUnavailableToast() {
    if (!orderUnavailableToast) {
      return;
    }
    if (orderToastHideTimer) {
      clearTimeout(orderToastHideTimer);
    }
    if (orderToastRemoveTimer) {
      clearTimeout(orderToastRemoveTimer);
    }
    orderUnavailableToast.setAttribute("aria-hidden", "false");
    orderUnavailableToast.classList.add("toast--visible");
    orderToastHideTimer = setTimeout(function () {
      orderUnavailableToast.classList.remove("toast--visible");
      orderUnavailableToast.setAttribute("aria-hidden", "true");
      orderToastRemoveTimer = setTimeout(function () {
        orderToastHideTimer = null;
        orderToastRemoveTimer = null;
      }, 320);
    }, 3200);
  }
  ...
  function renderGrid() {
    ...
    RECIPES.forEach(function (recipe) {
      ...
      card.addEventListener("click", function (e) {
        if (e.target.closest(".card__fav") || e.target.closest(".card__order")) {
          return;
        }
        openThis();
      });
      ...
      orderBtn.className = "btn btn--primary card__order";
      orderBtn.textContent = "Order now";
      orderBtn.setAttribute("aria-label", "Order " + recipe.title);
      orderBtn.addEventListener("click", function (e) {
        e.stopPropagation();
        showOrderUnavailableToast();
      });
  ...
  function showTonight(recipe) {
    ...
    var tonightImgEl = document.getElementById("tonightImg");
    tonightImgEl.src = recipe.image;
    tonightImgEl.alt = recipe.title;
    tonightImgEl.referrerPolicy = "no-referrer";
  ...
  var btnOrder = document.getElementById("btnOrderNow");
  if (btnOrder) {
    btnOrder.addEventListener("click", showOrderUnavailableToast);
  }
